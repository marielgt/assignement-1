...
```{r results='hide'}
library(readr)
List_of_stimuli <- read_csv("List of stimuli.xlsx")
```
...
library(readr)
library(ggplot2)
PresuppositionType_data <- read_csv("PresuppositionType_data.csv")
...
```{r results='hide'}
library(dplyr)
data2 <- filter(data, Group == "ASD subjects")
```
